...
class base_scene {
	...
	public function __construct($id = 0) {
		...
		$this->attr[] = $this->url = new attributo("url");
		$this->attr[] = $this->durata = new attributo("durata", "int");
		...
		if($id > 0){
			...
			if(count($arr) > 0){
				...
				$this->url->set_valore($arr[0]["url"]);
				$this->durata->set_valore($arr[0]["durata"]);
	...
	public function salva($make_log = true){
		...
		if($this->verifica_obbligatori() && !$this->errore){
			...
			if($this->id == 0){
				...
				$sql = "INSERT INTO scene (url, durata, live, data_start, genere, eta, razza) VALUES (?, ?, ?, ?, ?, ?, ?)";

				$dati_query = array(
								$this->url->get_valore(99), 
								$this->durata->get_valore(99), 
				...
				$sql = "UPDATE scene SET url = ?, durata = ?, live = ?, data_start = ?, genere = ?, eta = ?, razza = ?"
						." WHERE id = ?";
	...
	public function set_url($url) {
		$this->url->set_valore($url);
		return $this->url->is_corretto();
	}
	 ...
	 * @return boolean true se il valore e' corretto, false altrimenti
	 */
	public function set_durata($durata) {
		$this->durata->set_valore($durata);
		return $this->durata->is_corretto();
	}
	...
	public function get_url($formattazione_dato = 1) {
		return $this->url->get_valore($formattazione_dato);
	}
	
	public function get_durata($formattazione_dato = 1) {
		return $this->durata->get_valore($formattazione_dato);
	}
