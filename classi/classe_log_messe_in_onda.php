...
class classe_log_messe_in_onda extends base_log_messe_in_onda {
	...
	/**
	 * restituisce un array di oggetti classe_log_messe_in_onda con i risultati della query
	 * 
	 * @return \classe_log_messe_in_onda array con gli oggetti di tipo classe_log_messe_in_onda
	 */
	public function elenco_ricerca(){
		...
		$sql = "SELECT id AS id_ricerca FROM log_messe_in_onda";
		...
		foreach ($arr as $value) {
			...
			try{
				$risultati[] = new classe_log_messe_in_onda($value["id_ricerca"]);
			}catch (Exception $e){}
...
class base_log_messe_in_onda {
	...
	public function __construct($id = 0) {
		...
		$this->attr[] = $this->evento_id = new attributo("evento_id", "int");
		$this->attr[] = $this->scena_id = new attributo("scena_id");
		$this->attr[] = $this->created_at = new attributo("created_at");
		$this->attr[] = $this->updated_at = new attributo("updated_at");

		if($id > 0){

			$sql = "SELECT * FROM log_messe_in_onda WHERE id = ?";
			...
			if(count($arr) > 0){
				...
				$this->evento_id->set_valore($arr[0]["evento_id"]);
	...
	protected function delete($make_log = true){
		...
		if($make_log === true){
			...
			if($log->enabled()){
				$log->set_nome_azione("delete log_messe_in_onda");
	...
	public function salva($make_log = true){
		...
		if($this->verifica_obbligatori() && !$this->errore){
			...
			if($this->id == 0){
				...
				$sql = "INSERT INTO log_messe_in_onda (evento_id, scena_id, created_at, updated_at) VALUES (?, ?, ?, ?)";

				$dati_query = array(
								$this->evento_id->get_valore(99), 
				...
				if($make_log === true){
					...
					if($log->enabled()){
						$log->set_nome_azione("insert log_messe_in_onda");
				...
				$sql = "UPDATE log_messe_in_onda SET evento_id = ?, scena_id = ?, created_at = ?, updated_at = ?"
						." WHERE id = ?";
				...
				if($make_log === true){
					...
					if($log->enabled()){
						$log->set_nome_azione("update log_messe_in_onda");
	 ...
	 */
	public function set_evento_id($evento_id) {
		$this->evento_id->set_valore($evento_id);
		return $this->evento_id->is_corretto();
	}
	...
	public function get_evento_id($formattazione_dato = 1) {
		return $this->evento_id->get_valore($formattazione_dato);
	}
