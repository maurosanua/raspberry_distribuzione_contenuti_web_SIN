...
";
	...
	private $tipologia = "PgSQL PDO";
	private $codifica_db = null;//"utf8";  //specifica la codifica con cui vengono trasferiti i dati tra PHP e MySQL, per usare quello di default indicare NULL
	...
	function __construct($host = "", $user = "", $pw = "", $db = "",$port = ""){
		
		if(strlen($host) <= 0){$host = DB_HOST;}
		if(strlen($user) <= 0){$user = DB_USER;}
		if(strlen($pw) <= 0){$pw = DB_PASSWORD;}
		if(strlen($db) <= 0){$db = DB_NAME;}
		if(strlen($port) <= 0){$port = DB_PORT;}
		
		
		try{
			$this->DB_conn = new PDO("pgsql:host=".$host.";port=".$port.";dbname=".$db.($this->codifica_db !== null ? ";charset=".$this->codifica_db : ""), $user, $pw);
	...
	public function query_risultati($query, $dati = null){
		...
		try{
			//$this->fix_limit($query)  per convertire la sintassi mysql in pgsql
	...
	public function ultimo_id(){
		...
		$query = "select LASTVAL() as ultimoid";
...
//				var_dump($dati);
	...
	public static function fix_limit($query){
		$query_arr = preg_split("/ limit /i", $query);
		$out = $query_arr[0];
		if(count($query_arr)>1){
			for($i=1;$i<count($query_arr);$i++){
				$out.=" OFFSET ".preg_replace("/,/i",' LIMIT ',$query_arr[$i], 1);
			}
		}
		return $out;
	}
